...
public class PopBufferMergeService extends ServiceThread {
        ...
        while (iterator.hasNext()) {
            ...
            while ((pointWrapper = queue.peek()) != null) {
                ...
                if ((pointWrapper.isJustOffset() && pointWrapper.isCkStored()) || isCkDone(pointWrapper)
                    ...
                    if (System.currentTimeMillis() - pointWrapper.getCk().getPt()
    ...
    public long getLatestOffset(String topic, String group, int queueId) {
        QueueWithTime<PopCheckPointWrapper> queue = this.commitOffsets.get(
                KeyBuilder.buildPollingKey(topic, group, queueId)
        );
        if (queue == null) {
            return -1;
        }
    ...
    private boolean commitOffset(final PopCheckPointWrapper wrapper) {
        ...
        try {
            final long offset = brokerController.getConsumerOffsetManager().queryOffset(popCheckPoint.getC(), popCheckPoint.getT(), popCheckPoint.getQ());
            ...
            brokerController.getConsumerOffsetManager().commitOffset(getServiceName(),
                    popCheckPoint.getC(), popCheckPoint.getT(), popCheckPoint.getQ(), wrapper.getNextBeginOffset());
        } finally {
    ...
    private boolean putOffsetQueue(PopCheckPointWrapper pointWrapper) {
        ...
        queue.setTime(pointWrapper.getCk().getPt());
...
//            }
    ...
    private boolean putAckToStore(final PopCheckPointWrapper pointWrapper, byte msgIndex) {
        ...
        MsgUtil.setMessageDeliverTime(this.brokerController, msgInner, point.getRt());
    ...
    public class PopCheckPointWrapper {
        ...
        public PopCheckPointWrapper(int reviveQueueId, long reviveQueueOffset, PopCheckPoint point, long nextBeginOffset) {
            ...
            this.lockKey = ck.getT() + PopAckConstants.SPLIT + ck.getC() + PopAckConstants.SPLIT + ck.getQ();
            this.mergeKey = point.getT() + point.getC() + point.getQ() + point.getSo() + point.getPt();
