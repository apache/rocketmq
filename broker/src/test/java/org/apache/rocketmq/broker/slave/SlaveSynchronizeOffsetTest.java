...
/**
 * @ProjectName: rocketmq-source
 * @Package: org.apache.rocketmq.broker.slave
 * @ClassName: SlaveSynchronizeOffsetTest
 * @Author: wenjuan.bao
 * @Description:
 * @Date: 2024/9/18 17:34
 * @Version: 1.0
 */

@RunWith(MockitoJUnitRunner.class)
public class SlaveSynchronizeOffsetTest {
    ...
    @Test
    public void testSlaveSynchronizeOffset() throws RemotingConnectException, RemotingSendRequestException, RemotingTimeoutException, MQBrokerException, InterruptedException, UnsupportedEncodingException {
        when(this.brokerController.getMessageStoreConfig().isEnableLmq()).thenReturn(true);
        when(this.brokerController.getConsumerOffsetManager()).thenReturn(new LmqConsumerOffsetManager(this.brokerController));
        ...
        Long offset = this.brokerController.getConsumerOffsetManager().getOffsetTable().get(topic + ConsumerOffsetManager.TOPIC_GROUP_SEPARATOR + group).get(0);
        Assert.assertEquals(10L, (long)offset);

        String lmqTopic = "%LMQ%NewTopicA";
        String lmqGroup = "%LMQ%NewGroupA";
        Long lmqOffset = ((LmqConsumerOffsetManager) this.brokerController.getConsumerOffsetManager()).getLmqOffsetTable().get(lmqTopic + ConsumerOffsetManager.TOPIC_GROUP_SEPARATOR + lmqGroup);
        Assert.assertEquals(12L, (long)lmqOffset);
    }
    ...
    private ConsumerOffsetSerializeWrapper createConsumerOffsetWrapper() {
        ...
        String topic = "NewTopic";
        String group = "NewGroup";
        ConcurrentMap<Integer, Long> offset = new ConcurrentHashMap<>();
        offset.put(0, 10L);
        offsetTable.put(topic + ConsumerOffsetManager.TOPIC_GROUP_SEPARATOR + group, offset);
        wrapper.setOffsetTable(offsetTable);

        ConcurrentHashMap<String, Long> lmqOffsetTable = new ConcurrentHashMap<>();

        String lmqTopic = "%LMQ%NewTopicA";
        String lmqGroup = "%LMQ%NewGroupA";
        lmqOffsetTable.put(lmqTopic + ConsumerOffsetManager.TOPIC_GROUP_SEPARATOR + lmqGroup, 12L);
        wrapper.setLmqOffsetTable(lmqOffsetTable);
