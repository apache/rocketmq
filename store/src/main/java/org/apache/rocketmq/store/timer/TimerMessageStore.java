...
public class TimerMessageStore {
    ...
    public void recover() {
        ...
        if (-1 == queueOffset) {
            ...
            log.info("Timer recover if currQueueOffset:{} queueOffset is {}", currQueueOffset, queueOffset);
        } else {
            currQueueOffset = queueOffset + 1;
            log.info("Timer recover else currQueueOffset:{} queueOffset is {}", currQueueOffset, queueOffset);
        }
        log.info("Timer recover math min currQueueOffset:{} queueOffset is {}", currQueueOffset, timerCheckpoint.getMasterTimerQueueOffset());
    ...
    public boolean enqueue(int queueId) {
        ...
        if (currQueueOffset < cq.getMinOffsetInQueue()) {
            log.info("Timer currQueueOffset:{} is smaller than minOffsetInQueue:{}", currQueueOffset, cq.getMinOffsetInQueue());
            currQueueOffset = cq.getMinOffsetInQueue();
        } else if (currQueueOffset > cq.getMaxOffsetInQueue()) {
            log.info("Timer currQueueOffset:{} is larger than maxOffsetInQueue:{}", currQueueOffset, cq.getMaxOffsetInQueue());
        ...
        log.info("Timer EnqueueGetService enqueue bufferCQ {}, currQueueOffset {} offset result {}", bufferCQ, currQueueOffset, (offset * CQ_STORE_UNIT_SIZE) > cq.getMinLogicOffset());
        if (null == bufferCQ) {
            return false;
        }
        try {
