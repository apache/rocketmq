...
public class RouteServiceTest extends BaseServiceTest {
    ...
    public void testLocalModeQueryRoute() throws Exception {
        RouteService routeService = new RouteService(this.serviceManager, this.grpcClientManager);
        routeService.start();
    ...
    @Override public void beforeEach() throws Throwable {
        ...
        MessageQueueView messageQueueView = new MessageQueueView("topic", routeData);
        when(this.topicRouteService.getAllMessageQueueView("topic")).thenReturn(messageQueueView);

        when(this.topicRouteService.getAllMessageQueueView("notExistTopic")).thenThrow(new MQClientException(ResponseCode.TOPIC_NOT_EXIST, ""));
    }
}