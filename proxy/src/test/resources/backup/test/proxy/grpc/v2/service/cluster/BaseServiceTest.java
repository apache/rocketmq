...
import org.apache.rocketmq.proxy.service.DefaultForwardClient;
import org.apache.rocketmq.proxy.service.ForwardProducer;
import org.apache.rocketmq.proxy.service.ForwardReadConsumer;
...
public abstract class BaseServiceTest extends InitConfigAndLoggerTest {
    ...
    public void before() throws Throwable {
        super.before();
        when(serviceManager.getDefaultForwardClient()).thenReturn(defaultClient);
        when(serviceManager.getForwardProducer()).thenReturn(producerClient);
        when(serviceManager.getForwardReadConsumer()).thenReturn(readConsumerClient);
        when(serviceManager.getForwardWriteConsumer()).thenReturn(writeConsumerClient);
        when(serviceManager.getTopicRouteService()).thenReturn(topicRouteService);
        when(serviceManager.getTransactionHeartbeatRegisterService()).thenReturn(clusterTransactionService);
