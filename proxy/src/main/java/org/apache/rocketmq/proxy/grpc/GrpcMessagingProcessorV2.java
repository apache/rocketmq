...
import org.apache.rocketmq.proxy.grpc.service.GrpcForwardServiceV2;

public class GrpcMessagingProcessorV2 extends MessagingServiceGrpc.MessagingServiceImplBase {

    private final GrpcForwardServiceV2 grpcForwardService;

    public GrpcMessagingProcessorV2(GrpcForwardServiceV2 grpcForwardService) {
        this.grpcForwardService = grpcForwardService;
    }

    public Status convertExceptionToStatus(Throwable t) {
        if (t instanceof CompletionException) {
            if (t.getCause() instanceof ProxyExceptionV2) {
                ProxyExceptionV2 proxyException = (ProxyExceptionV2) t.getCause();
                return ResponseBuilderV2.buildStatus(proxyException.getCode(), proxyException.getMessage());
            }
        }
        return ResponseBuilderV2.buildStatus(Code.INTERNAL_SERVER_ERROR, "internal error");
    }
