...
public class SelectMessageQueueByConsistentHash implements MessageQueueSelector {
    ...
    private void rehash(List<MessageQueue> mqs) {
        //newly added
        List<MQNode> newMq = new ArrayList<MQNode>();
        for (MessageQueue mq : mqs) {
            if (!lastMqs.contains(mq)) {
                newMq.add(new MQNode(mq));
            }
        }

        //remove already
        List<MQNode> removeMq = new ArrayList<MQNode>();
        for (MessageQueue last : lastMqs) {
            if (!mqs.contains(last)) {
                removeMq.add(new MQNode(last));
            }
        }

        for (MQNode mqNode : newMq) {
            consistentHashRouter.addNode(mqNode, virtualNodeNum);
        }

        for (MQNode mqNode : removeMq) {
            consistentHashRouter.removeNode(mqNode);
        }
    ...
    class MQNode implements Node {
        private final MessageQueue mq;

        public MQNode(MessageQueue mq) {
            this.mq = mq;
        }
