...
public abstract class NettyRemotingAbstract {
     ...
     */
    protected Pair<NettyRequestProcessor, ExecutorService> defaultRequestProcessor; //消息处理器
    ...
    public RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request,
        ...
        final int opaque = request.getOpaque(); //获取请求Id

        try {
            final ResponseFuture responseFuture = new ResponseFuture(channel, opaque, timeoutMillis, null, null);
            this.responseTable.put(opaque, responseFuture);
            final SocketAddress addr = channel.remoteAddress(); //通过channel获取 SocketAddress
    ...
    public void invokeAsyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis,
        ...
        boolean acquired = this.semaphoreAsync.tryAcquire(timeoutMillis, TimeUnit.MILLISECONDS); //异步发送用信息量作为并发控制
        if (acquired) {
    ...
    public void invokeOnewayImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis)
        ...
        boolean acquired = this.semaphoreOneway.tryAcquire(timeoutMillis, TimeUnit.MILLISECONDS); //申请信号量
        if (acquired) {
