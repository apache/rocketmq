...
public class StaticTopicIT extends BaseConf {
    ...
    public void testRemappingAndClear() throws Exception {
        ...
        {
            Set<String> topics =  new HashSet<>(BROKER_CONTROLLER1.getTopicConfigManager().getTopicConfigTable().keySet());
            topics.remove(topic);
            BROKER_CONTROLLER1.getMessageStore().cleanUnusedTopic(topics);
            BROKER_CONTROLLER1.getMessageStore().cleanUnusedTopic(topics);
            for (int i = 0; i < 10; i++) {
                for (BrokerController brokerController: BROKER_CONTROLLER_LIST) {
                    brokerController.getTopicQueueMappingCleanService().wakeup();
