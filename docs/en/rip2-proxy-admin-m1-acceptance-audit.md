...
| Paginated query P99 latency < 1s at 1M clients without OOM risk. | Locally verified. | `docs/en/rip2-proxy-admin-m1-benchmark-report.md` records worst local read-model P99 at 0.681 ms on 1M synthetic clients after the single-index read-path optimization. | Re-run if query-path implementation changes. |
