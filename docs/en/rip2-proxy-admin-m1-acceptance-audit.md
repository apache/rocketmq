...
| Paginated query P99 latency < 1s at 1M clients without OOM risk. | Latency locally verified; worst-case memory proof pending. | `docs/en/rip2-proxy-admin-m1-benchmark-report.md` records worst local read-model P99 at 0.681 ms and worst generated public gRPC endpoint P99 at 3.576 ms on 1M synthetic clients. | Record peak heap, allocation, and GC evidence for broad-prefix, combined-filter, deep-page, and production-executor scenarios under constrained heap. |
