...
3. Add a coordinator service that fans out local-page requests, merges results in
   ...
   rejects blank or duplicate peer ids as internal discovery errors, and
   validates each peer response `proxyId` before accepting its body so a
   misrouted or stale peer transport result is returned as an internal routing
   error instead of being merged into the admin response.
