...
| ACL 2.0 使用独立 `proxy.admin.client` 资源。 | 已实现并测试。 | 单测覆盖 policy 构造与求值。`dev/run_rip2_authenticated_smoke.sh` 还会在 authentication/authorization 均开启的完整 distribution 上证明 LIST-only 用户可 list 但不能 describe，GET-only 用户可 describe 但不能 list，资源统一为 `Admin:proxy.admin.client`。 | 资源命名仍待社区 review。 |
