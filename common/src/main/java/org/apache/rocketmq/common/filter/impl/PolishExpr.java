...
import static org.apache.rocketmq.common.filter.impl.Operator.LEFT_PARENTHESIS;
import static org.apache.rocketmq.common.filter.impl.Operator.RIGHT_PARENTHESIS;
...
public class PolishExpr {
    ...
    public static List<Op> reversePolish(List<Op> tokens) {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (isOperand(token)) {
                ...
                Operator opNew = null;
                while (!operatorStack.empty() && LEFT_PARENTHESIS != (opNew = operatorStack.pop())) {
                    segments.add(opNew);
                }
                if (null == opNew || LEFT_PARENTHESIS != opNew)
        ...
        while (!operatorStack.empty()) {
            ...
            if (LEFT_PARENTHESIS == operator || RIGHT_PARENTHESIS == operator)
    ...
    public static boolean isLeftParenthesis(Op token) {
        return token instanceof Operator && LEFT_PARENTHESIS == (Operator) token;
    }

    public static boolean isRightParenthesis(Op token) {
        return token instanceof Operator && RIGHT_PARENTHESIS == (Operator) token;
    }
