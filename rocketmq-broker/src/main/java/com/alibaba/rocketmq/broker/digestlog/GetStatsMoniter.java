...
public class GetStatsMoniter {
    ...
    private ConcurrentHashMap<String/* topic,partition,cliHostName,group */, GetStatsInfo> getStatsInfos =
            new ConcurrentHashMap<String, GetStatsInfo>();


    public void append(String topic, String partition, String cliHostName, String group, long succesCount,
            long faileCount, long timeoutCount, long excTime, long msgSize) {
        GetStatsInfo getStatsInfo = getStatsInfos.get(topic + partition + cliHostName + group);
        if (null == getStatsInfo) {
            getStatsInfo =
                    new GetStatsInfo(topic, partition, cliHostName, group, succesCount, faileCount,
                        timeoutCount, excTime, msgSize);
