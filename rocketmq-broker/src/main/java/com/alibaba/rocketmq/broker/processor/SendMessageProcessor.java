    ...
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        if (putMessageResult != null) {
            ...
            if (sendOK) {
                ...
                this.brokerController.getDigestLogManager().getPutStatsMoniter().append(msgInner.getTopic(),this.brokerController.getBrokerConfig().getBrokerName()+ "-" +msgInner.getQueueId(),msgInner.getBornHostNameString(), 1, 0, 0, System.currentTimeMillis()-msgInner.getBornTimestamp(), msgInner.getBody().length);
                this.brokerController.getDigestLogManager().getStoreStatsMoniter().putAppend(msgInner.getTopic(), this.brokerController.getBrokerConfig().getBrokerName()+ "-" +msgInner.getQueueId(), putMessageResult.getAppendMessageResult().getLogicsOffset());
                return null;
            }
        }
        else {
            ...
            this.brokerController.getDigestLogManager().getPutStatsMoniter().append(msgInner.getTopic(),this.brokerController.getBrokerConfig().getBrokerName()+ "-" +msgInner.getQueueId(),msgInner.getBornHostNameString(), 0, 1, 0, System.currentTimeMillis()-msgInner.getBornTimestamp(), msgInner.getBody().length);
        }
