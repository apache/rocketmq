...
public class MigrateTopicLogicalQueueCommand implements SubCommand {
    ...
    @Override public Options buildCommandlineOptions(Options options) {
        ...
        opt = new Option("f", "forceDelta", true, "assume fromBroker down, force migrate");
        opt.setRequired(false);
        options.addOption(opt);

        // TODO add option to specify logicalQueueDelta for force migrating(skip failed broker)

        return options;
    }

    public void execute(DefaultMQAdminExt mqAdminExt, String topic, int logicalQueueIdx,
        String toBrokerName) throws RemotingException, MQBrokerException, InterruptedException, SubCommandException, MQClientException {
    ...
    @Override public void execute(CommandLine commandLine, Options options,
        ...
        try {
            ...
            execute(mqAdminExt, topic, logicalQueueIdx, newBrokerName);
        } catch (Exception e) {
