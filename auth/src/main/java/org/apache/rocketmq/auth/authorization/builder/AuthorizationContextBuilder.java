...
public class AuthorizationContextBuilder {
    ...
    public AuthorizationContextBuilder(AuthConfig authConfig) {
        this.authConfig = authConfig;
        this.requestHeaderRegistry = RequestHeaderRegistry.getInstance();
    }

    public List<AuthorizationContext> build(Metadata metadata, GeneratedMessageV3 message) {
        if (message instanceof SendMessageRequest) {
            ...
            return newPubContext(metadata, request.getMessages(0).getTopic());
        }
        if (message instanceof EndTransactionRequest) {
            EndTransactionRequest request = (EndTransactionRequest) message;
            return newPubContext(metadata, request.getTopic());
        }
        if (message instanceof HeartbeatRequest) {
            ...
            return newGroupSubContexts(metadata, request.getGroup());
        }
        ...
        if (message instanceof AckMessageRequest) {
            ...
            return newSubContexts(metadata, request.getGroup(), request.getTopic());
        }
        ...
        if (message instanceof QueryRouteRequest) {
            ...
            return newContext(metadata, request);
        }
        ...
        throw new AuthorizationException("The current API lacks definitions for authorization");
    }

    public List<AuthorizationContext> build(RemotingCommand request, String remoteAddr) {
    ...
    private List<AuthorizationContext> buildContextByAnnotation(Subject subject, RemotingCommand request,
        String sourceIp) throws Exception {
        List<AuthorizationContext> result = new ArrayList<>();
    ...
    private List<AuthorizationContext> newContext(Metadata metadata, QueryRouteRequest request) {
        ...
        AuthorizationContext context = AuthorizationContext.of(subject, resource, Arrays.asList(Action.PUB, Action.SUB), sourceIp);
        return Collections.singletonList(context);
    }

    private static List<AuthorizationContext> newContext(Metadata metadata, TelemetryCommand request) {
    ...
    private static List<AuthorizationContext> newPubContext(Metadata metadata, apache.rocketmq.v2.Resource topic) {
        ...
        Resource resource = Resource.ofTopic(topic.getName());
        String sourceIp = metadata.get(GrpcConstants.REMOTE_ADDRESS);
        AuthorizationContext context = AuthorizationContext.of(subject, resource, Action.PUB, sourceIp);
        return Collections.singletonList(context);
    }

    private List<AuthorizationContext> newSubContexts(Metadata metadata, apache.rocketmq.v2.Resource group,
    ...
    private static List<AuthorizationContext> newTopicSubContexts(Metadata metadata,
        apache.rocketmq.v2.Resource resource) {
        return newSubContexts(metadata, ResourceType.TOPIC, resource);
    }

    private static List<AuthorizationContext> newGroupSubContexts(Metadata metadata,
        apache.rocketmq.v2.Resource resource) {
        return newSubContexts(metadata, ResourceType.GROUP, resource);
    }

    private static List<AuthorizationContext> newSubContexts(Metadata metadata, ResourceType resourceType,
        apache.rocketmq.v2.Resource resource) {
    ...
    private static List<AuthorizationContext> newSubContexts(Metadata metadata, Resource resource) {
        List<AuthorizationContext> result = new ArrayList<>();
        User subject = User.of(metadata.get(GrpcConstants.AUTHORIZATION_AK));
        String sourceIp = metadata.get(GrpcConstants.REMOTE_ADDRESS);
        result.add(AuthorizationContext.of(subject, resource, Action.SUB, sourceIp));
        return result;
    }
}
